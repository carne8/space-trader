...
impl Display for GetSystemWaypointsTraitsParameter {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            GetSystemWaypointsTraitsParameter::WaypointTraitSymbol(s) => write!(f, "{}", s),
            GetSystemWaypointsTraitsParameter::Array(e) => write!(f, "{}", e.iter().map(|x| x.to_string()).collect::<Vec<String>>().join(", ")),
        }
    }
}
